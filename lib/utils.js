 ...
 */
export function collides(l1: LayoutItem, l2: LayoutItem, compactType?: CompactType,): boolean {
...
export function compactItem(
  ...
  if (compactV) {
    ...
    while (l.y > 0 && !getFirstCollision(compareWith, l, compactType)) {
      l.y--;
    }
  } else if (compactH) {
    // Move the element left as far as it can go without colliding.
    while (l.x > 0 && !getFirstCollision(compareWith, l, compactType)) {
      l.x--;
    }
  }
  ...
  while (
    (collides = getFirstCollision(compareWith, l, compactType)) &&
...
export function getFirstCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    if (collides(layout[i], layoutItem, compactType)) return layout[i];
  }
}
