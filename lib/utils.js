 ...
 */
export function collides(l1: LayoutItem, l2: LayoutItem, stepFrom = window?.step_global.rootWidget, collisionType?: string): boolean {
  const step = stepFrom?.x ? stepFrom.x / 2 - 1 :  32
  if (collisionType === 'strict') {
    if (l1.i === l2.i) return false; // same element
    if (l1.x + l1.w <= l2.x + 2) return false; // l1 is left of l2
    if (l1.x + 2 >= l2.x + l2.w) return false; // l1 is right of l2
    if (l1.y + l1.h <= l2.y + 2) return false; // l1 is above l2
    if (l1.y + 2 >= l2.y + l2.h) return false; // l1 is below l2
    return true;
  } else if (collisionType === 'resize') {
    if (l1.i === l2.i) return false; // same element
    if (l1.x + l1.w <= l2.x + step) return false; // l1 is left of l2
    if (l1.x + step >= l2.x + l2.w) return false; // l1 is right of l2
    if (l1.y + l1.h <= l2.y + 2) return false; // l1 is above l2
    if (l1.y + 2 >= l2.y + l2.h) return false; // l1 is below l2
    return true;
  } else if (collisionType === 'dragPlaceholder') {
     if (l1.i === l2.i) return false; // same element
    if (l1.x + l1.w <= l2.x + step) return false; // l1 is left of l2
    if (l1.x + step >= l2.x + l2.w) return false; // l1 is right of l2
    ...
    return true;
  }  else if (collisionType === 'drag') {
    if (l1.i === l2.i) return false; // same element
    if (l1.x + l1.w <= l2.x + step) return false; // l1 is left of l2
    if (l1.x + step >= l2.x + l2.w) return false; // l1 is right of l2
    if (l1.y + l1.h * 1 / 3 <= l2.y) return false; // l1 is above l2
    if (l1.y >= l2.y + l2.h * 1 / 3) return false; // l1 is below l2
    return true;
  } else {
    if (l1.i === l2.i) return false; // same element
    if (l1.x + l1.w <= l2.x + step) return false; // l1 is left of l2
...
export function compact(
  ...
  step = window?.step_global?.rootWidget,
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      l = compactItem(compareWith, l, compactType, cols, sorted, allowOverlap, collisionType, step);
...
function resolveCompactionCollision(
  ...
  for (let i = itemIndex + 1; i < layout.length; i++) {
    ...
    if (collides(item, otherItem, step, collisionType)) {
...
export function compactItem(
  ...
  if (compactV) {
    ...
    while (l.y > 0 && !getFirstCollision(compareWith, l, step, collisionType)) {
      l.y = l.y - step.y;
    }
  } else if (compactH) {
    // Move the element left as far as it can go without colliding.
    while (l.x > 0 && !getFirstCollision(compareWith, l, step, collisionType)) {
      l.x--;
    }
  }
  ...
  while (
    (collides = getFirstCollision(compareWith, l, step, collisionType)) &&
    !(compactType === null && allowOverlap)
  ) {
    if (compactH) {
      resolveCompactionCollision(fullLayout, l, collides.x + collides.w, "x", step, collisionType);
    } else {
      resolveCompactionCollision(fullLayout, l, collides.y + collides.h, "y", step, collisionType);
    }
...
export function correctBounds(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    ...
    if (!l.static) collidesWith.push(l);
      ...
      while (getFirstCollision(collidesWith, l, step)) {
        l.y++;
      }
    }
  }
...
export function getFirstCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    if (collides(layout[i], layoutItem, step, collisionType)) return layout[i];
  }
}
...
export function getAllCollisions(
  ...
  return layout.filter(l => collides(l, layoutItem, step, collisionType));
}
...
export function moveElement(
  ...
  const movingUp =
    (compactType === "vertical" || compactType === "null") && typeof y === "number"
      ? oldY >= y
  ...
  const collisions = getAllCollisions(sorted, l, step, collisionType);
  ...
  if (!hasCollisions) { return layout }
...
export function moveElementAwayFromCollision(
  ...
  if (isUserAction) {
    ...
    const firstCollision = getFirstCollision(layout, fakeItem, step, collisionType || 'strict');
    // RGL线上bug
    const collisionNorth =
      firstCollision && firstCollision.i !== collidesWith.i && firstCollision.y + firstCollision.h > collidesWith.y;
    const collisionWest =
      firstCollision  && firstCollision.i !== collidesWith.i && collidesWith.x + collidesWith.w > firstCollision.x;
  ...
  const newX = compactH ? itemToMove.x + step.x : undefined;
  const newY = compactV ? itemToMove.y + step.y : undefined;
  ...
  return moveElement(
    layout,
    itemToMove,
    compactH ? itemToMove.x + step.x : undefined,
    compactV ? itemToMove.y + step.y : undefined,
    isUserAction,
    preventCollision,
    compactType,
    cols,
...
export function synchronizeLayoutWithChildren(
  ...
  // Correct the layout.
  const correctedLayout = correctBounds(layout, { cols: cols }, step);
  console.log('====> synchronizeLayoutWithChildren', correctedLayout)
  return allowOverlap
    ? correctedLayout
    : compact(correctedLayout, compactType, cols, undefined, step);
}
