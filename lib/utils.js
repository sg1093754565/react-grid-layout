 ...
 */
export function collides(l1: LayoutItem, l2: LayoutItem, compactType?: CompactType, isCollisionStrict?: boolean): boolean {
  if (isCollisionStrict) {
    ...
    if (l1.i === l2.i) return false; // same element
    if (l1.x + l1.w <= l2.x + 32) return false; // l1 is left of l2
    if (l1.x + 32 >= l2.x + l2.w) return false; // l1 is right of l2
...
export function compact(
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      l = compactItem(compareWith, l, compactType, cols, sorted, allowOverlap, isCollisionStrict);
...
export function compactItem(
  ...
  if (compactV) {
    ...
    while (l.y > 0 && !getFirstCollision(compareWith, l, compactType, isCollisionStrict)) {
      l.y--;
    }
  } else if (compactH) {
    // Move the element left as far as it can go without colliding.
    while (l.x > 0 && !getFirstCollision(compareWith, l, compactType, isCollisionStrict)) {
      l.x--;
    }
  }
  ...
  while (
    (collides = getFirstCollision(compareWith, l, compactType, isCollisionStrict)) &&
...
export function getFirstCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    if (collides(layout[i], layoutItem, compactType, isCollisionStrict)) return layout[i];
  }
}
