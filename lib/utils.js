 ...
 */
export function collides(l1: LayoutItem, l2: LayoutItem, compactType?: CompactType, collisionType?: string): boolean {
  const step = window.step_global?.rootWidget?.x ? window.step_global?.rootWidget?.x / 2 - 1 : 32
...
export function compact(
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      l = compactItem(compareWith, l, compactType, cols, sorted, allowOverlap, collisionType);
...
export function compactItem(
  ...
  if (compactV) {
    ...
    while (l.y > 0 && !getFirstCollision(compareWith, l, compactType, collisionType)) {
      l.y--;
    }
  } else if (compactH) {
    // Move the element left as far as it can go without colliding.
    while (l.x > 0 && !getFirstCollision(compareWith, l, compactType, collisionType)) {
      l.x--;
    }
  }
  ...
  while (
    (collides = getFirstCollision(compareWith, l, compactType, collisionType)) &&
...
export function getFirstCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    if (collides(layout[i], layoutItem, compactType, collisionType)) return layout[i];
  }
}
...
export function getAllCollisions(
  ...
  return layout.filter(l => collides(l, layoutItem, compactType, collisionType));
}
...
export function moveElement(
  ...
  const collisions = getAllCollisions(sorted, l, compactType, 'strict');
  // 这里有个坑，判断有没有挤占跟挤占获取的数量之间逻辑是不一致的
  // bad case：当横向挤占时，其纵向的覆盖判断是会有问题的，导致拿到的挤占数量相比过去少了（因为纵向高度挤占逻辑的宽松）
  // 解决方案：区分有碰撞跟碰撞数量的方法，碰撞数量严格些，有碰撞宽松
  const hasCollisions = getAllCollisions(sorted, l, compactType, collisionType).length > 0;
...
export function moveElementAwayFromCollision(
  ...
  if (isUserAction) {
    ...
    const firstCollision = getFirstCollision(layout, fakeItem, compactType, collisionType || 'strict');
